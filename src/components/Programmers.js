...
const [activeList, setActiveList] = useState(listOfAwesome)
const [currentName, setCurrentName] = useState(null)
  ...
  for(let i = 0; i < activeList.length; i++) {
    if(activeList[i].id == currentName){
      return activeList[i].name
    }
  }
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentName ? 'gold' : 'royalblue'
  };
      ...
      <div className='programmers'>
        {
          ...
          activeList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setCurrentName(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
